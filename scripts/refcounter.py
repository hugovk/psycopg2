...
def main():
    ...
    for i in range(1, opt.nruns + 1):
        sys.stdout.write(f"test suite run {int(i)} of {int(opt.nruns)}\n")
        runner = unittest.TextTestRunner()
        runner.run(test.test_suite())
        dump(i, opt)

    f1 = open(f'debug-{int(opt.nruns - 1):02}.txt').readlines()
    f2 = open(f'debug-{int(opt.nruns):02}.txt').readlines()
    for line in difflib.unified_diff(f1, f2,
            f"run {int(opt.nruns - 1)}", f"run {int(opt.nruns)}"):
        sys.stdout.write(line)

    rv = f1 != f2 and 1 or 0

    if opt.objs:
        f1 = open(f'objs-{int(opt.nruns - 1):02}.txt').readlines()
        f2 = open(f'objs-{int(opt.nruns):02}.txt').readlines()
        for line in difflib.unified_diff(f1, f2,
                f"run {int(opt.nruns - 1)}", f"run {int(opt.nruns)}"):
            sys.stdout.write(line)
...
def dump(i, opt):
    ...
    pprint(
        ...
        stream=open(f"debug-{int(i):02}.txt", "w"))
    ...
    if opt.objs:
        ...
        pprint(co, stream=open(f"objs-{int(i):02}.txt", "w"))
