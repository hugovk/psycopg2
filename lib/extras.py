...
class MinTimeLoggingConnection(LoggingConnection):
    ...
    def filter(self, msg, curs):
        ...
        if t > self._mintime:
            if isinstance(msg, bytes):
                msg = msg.decode(_ext.encodings[self.encoding], 'replace')
            return msg + _os.linesep + f"  (execution time: {int(t)} ms)"
...
class ReplicationCursor(_replicationCursor):
    ...
    def start_replication(
        ...
        if timeline != 0:
            ...
            command += f" TIMELINE {int(timeline)}"
...
class HstoreAdapter:
    ...
    def parse(self, s, cur, _bsdec=_re.compile(r"\\(.)")):
        ...
        for m in self._re_hstore.finditer(s):
            if m is None or m.start() != start:
                raise psycopg2.InterfaceError(
                    f"error parsing hstore pair at char {int(start)}")
        ...
        if start < len(s):
            raise psycopg2.InterfaceError(
                f"error parsing hstore: unparsed data after char {int(start)}")
